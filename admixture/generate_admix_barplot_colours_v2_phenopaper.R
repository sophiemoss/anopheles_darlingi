...
plot_gg_admix <- function(df, label_id, label_cat, type, palette, l_angle) {
  p <- ggplot(data = df, aes(x = !!sym(label_id), y = Q, fill = K)) +
    ...
    theme(
      legend.position = "top",
      strip.text.x = element_text(size = 12, face = "bold", angle = 0, hjust = 0.5),
      panel.spacing.x = unit(0.5, "lines"),
      axis.text.x = element_text(angle = l_angle, size = 8, hjust = 1, vjust = 0.5),
...
#Debug statement
print("Expected columns (met_cols):")
print(met_cols)
