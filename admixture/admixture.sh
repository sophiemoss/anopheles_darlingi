...
zgrep -v "^#" F_MISSING_MAF_AC0_DP5_GQ15_gatk_filtered_minac_filtrenamedchr_bi_snps_darlingi.genotyped.vcf.gz | cut -f1 | sort | uniq

zcat F_MISSING_MAF_AC0_DP5_GQ15_gatk_filtered_minac_filtrenamedchr_bi_snps_darlingi.genotyped.vcf.gz | awk 'BEGIN{OFS=FS="\t"} /^#/ {print; next} {gsub(/^anop_mito$/, "5", $1); gsub(/^anop_X$/, "6", $1); print}' | bgzip > admixture_modified.vcf.gz
...
plink --vcf admixture_modified.vcf.gz --set-missing-var-ids @:# --keep-allele-order --const-fid --allow-extra-chr --make-bed --out darlingi


# STEP 4 RUN ADMIXTURE

cat K_runs.txt | xargs -I {} sh -c 'admixture --cv=10 -j20 -s 14062 darlingi.bed {} | tee log{}.cv10.seed14062.out'
...
-d /mnt/storage11/sophie/darlingi/holly_wgs_paper \
--prefix darlingi \
--kval 2 \
