...
alphacyp_res_samples = df_samples[df_samples['resistance_status'] == 'alpha-cypermethrin-resistant'].index.values
alphacyp_res_samples

# %%
## select genotypes for resistant samples

gt_res_samples = gt.take(alphacyp_res_samples, axis=1)
gt_res_samples
...
alphacyp_sus_samples = df_samples[df_samples['resistance_status'] == 'alpha-cypermethrin-susceptible'].index.values
alphacyp_sus_samples

# %%
## select genotypes for resistant samples

gt_sus_samples = gt.take(alphacyp_sus_samples, axis=1)
gt_sus_samples
...
xpehh_positions_above_threshold_5 = pos[xpehh_raw >= 5]

# Save positions_above_threshold to a text file
with open("xpehh_positions_above_threshold_5", "w") as file:
    for position in xpehh_positions_above_threshold_5:
        file.write(str(position) + "\n")
